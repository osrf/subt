def spawner(_name, _modelURI, _worldName, _x, _y, _z, _roll, _pitch, _yaw)
  ...
  <spawn name='#{_name}'>
    ...
    <pose>#{_x} #{_y} #{_z + 0.1275} #{_roll} #{_pitch} #{_yaw}</pose>
...
def rosExecutables(_name, _worldName)
  ...
  <executable name='robot_description'>
      <command>roslaunch --wait coro_jeanine_sensor_config_1 description.launch world_name:=#{_worldName} name:=#{_name}</command>
    </executable>
    <executable name='topics'>
      <command>roslaunch --wait coro_jeanine_sensor_config_1 vehicle_topics.launch world_name:=#{_worldName} name:=#{_name} breadcrumbs:=1</command>
    </executable>
  HEREDOC
end
