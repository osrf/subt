...
def rosExecutables(_name, _worldName)
  <<-HEREDOC
    <executable name='x4_description'>
      <command>roslaunch --wait dynamo_x4_sensor_config_1 description.launch world_name:=#{_worldName} name:=#{_name}</command>
    </executable>
    <executable name='x4_ros_ign_bridge'>
      <command>roslaunch --wait dynamo_x4_sensor_config_1 vehicle_topics.launch world_name:=#{_worldName} name:=#{_name} uav:=1 lidar_3d:=1 laser_scan:=0 top_scan:=1 bottom_scan:=1 stereo_cam:=0 rgbd_cam:=0    </command>
  </executable>
  HEREDOC
end
