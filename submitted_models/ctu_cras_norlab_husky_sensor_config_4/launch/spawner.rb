...
def spawner(_name, _modelURI, _worldName, _x, _y, _z, _roll, _pitch, _yaw)
  base_spawn = `rospack find ctu_cras_norlab_husky_sensor_config_3`.chomp + "/launch/common.rb"
        ...
        <breadcrumb>
          <sdf version="1.6">
            <model name="#{_name}__breadcrumb__">
              <pose>0.60 0.18 0.05 0.33 0 #{Math::PI/2}</pose>
              <include>
...
def rosExecutables(_name, _worldName)
  ...
  <executable name='topics'>
    <command>roslaunch --wait ctu_cras_norlab_husky_sensor_config_3 vehicle_topics.launch world_name:=#{_worldName} name:=#{_name} breadcrumbs:=1 description_print_command:="#{File.dirname(__FILE__) + '/../scripts/print_robot_urdf'}"</command>
  </executable>
  HEREDOC
end
