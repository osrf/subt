def spawner(_name, _modelURI, _worldName, _x, _y, _z, _roll, _pitch, _yaw)
  ...
  <spawn name='#{_name}'>
      ...
      <pose>#{_x} #{_y} #{_z + 0.155} #{_roll} #{_pitch} #{_yaw}</pose>
      ...
      <plugin
        ...
        <maximumLinearVelocity>6.5 6 3</maximumLinearVelocity>
...
def rosExecutables(_name, _worldName)
  <<-HEREDOC
    <executable name='bandicoot_description'>
      <command>roslaunch --wait coro_bandicoot_sensor_config_1 description.launch world_name:=#{_worldName} name:=#{_name}</command>
    </executable>
    <executable name='bandicoot_ros_ign_bridge'>
      <command>roslaunch --wait coro_bandicoot_sensor_config_1 vehicle_topics.launch world_name:=#{_worldName} name:=#{_name} uav:=1 laser_scan:=1 top_scan:=0 bottom_scan:=1 rgbd_cam:=1    </command>
  </executable>
  HEREDOC
end
