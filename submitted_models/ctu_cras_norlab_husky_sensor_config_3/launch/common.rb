def _spawner(_name, _modelURI, _worldName, _x, _y, _z, _roll, _pitch, _yaw, _additionalSDF, _max_velocity=1, _max_acceleration=3)
  <<-HEREDOC
  <spawn name='#{_name}'>
    <name>#{_name}</name>
...
def _rosExecutables(_name, _worldName)
  ...
  <executable name='topics'>
    <command>roslaunch --wait ctu_cras_norlab_husky_sensor_config_3 vehicle_topics.launch world_name:=#{_worldName} name:=#{_name}</command>
  </executable>
  HEREDOC
end
