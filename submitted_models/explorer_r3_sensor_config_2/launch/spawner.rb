  ...
  <spawn name='#{_name}'>
    ...
    <include>
      ...
      <plugin filename="libignition-gazebo-diff-drive-system.so"
        ...
        <wheel_separation>#{0.320 * 2.0 - 0.0765}</wheel_separation>
      ...
      <plugin filename="libignition-gazebo-wheel-slip-system.so"
        ...
        <wheel link_name="left_front_wheel_link">
          <slip_compliance_lateral>0.0045</slip_compliance_lateral>
          <slip_compliance_longitudinal>0</slip_compliance_longitudinal>
          <wheel_normal_force>190.461</wheel_normal_force>
          <wheel_radius>0.246</wheel_radius>
        </wheel>
        <wheel link_name="left_rear_wheel_link">
          <slip_compliance_lateral>0.007</slip_compliance_lateral>
          <slip_compliance_longitudinal>0</slip_compliance_longitudinal>
          <wheel_normal_force>295.524</wheel_normal_force>
          <wheel_radius>0.246</wheel_radius>
        </wheel>
...
def rosExecutables(_name, _worldName)
  ...
  <executable name='topics'>
    <command>roslaunch --wait explorer_r3_sensor_config_1 vehicle_topics.launch world_name:=#{_worldName} name:=#{_name} breadcrumbs:=1</command>
  </executable>
  HEREDOC
end
