    ...
    <include>
      ...
      <plugin filename="libignition-gazebo-wheel-slip-system.so"
        ...
        <wheel link_name="front_left_wheel_link">
          ...
          <wheel_normal_force>149.6</wheel_normal_force>
          <wheel_radius>0.1651</wheel_radius>
        </wheel>
...
def rosExecutables(_name, _worldName)
  <<-HEREDOC
    <executable name='robot_description'>
      <command>roslaunch --wait ctu_cras_norlab_husky_sensor_config_1 description.launch world_name:=#{_worldName} name:=#{_name}</command>
    </executable>
    <executable name='topics'>
      <command>roslaunch --wait ctu_cras_norlab_husky_sensor_config_1 vehicle_topics.launch world_name:=#{_worldName} name:=#{_name} breadcrumbs:=12</command>
    </executable>
    HEREDOC
end
