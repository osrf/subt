...
def rosExecutables(_name, _worldName)
  <<-HEREDOC
    <executable name='x4_description'>
      <command>roslaunch --wait ssci_x4_sensor_config_2 description.launch world_name:=#{_worldName} name:=#{_name}</command>
    </executable>
    <executable name='x4_ros_ign_bridge'>
      <command>roslaunch --wait ssci_x4_sensor_config_2 vehicle_topics.launch world_name:=#{_worldName} name:=#{_name} uav:=1 laser_scan:=1 top_scan:=1 bottom_scan:=1 stereo_cam:=0 rgbd_cam:=1    </command>
  </executable>
  HEREDOC
end
