...
def rosExecutables(_name, _worldName)
  <<-HEREDOC
    <executable name='x4_description'>
      <command>roslaunch --wait marble_qav500_sensor_config_2 description.launch world_name:=#{_worldName} name:=#{_name}</command>
    </executable>
    <executable name='x4_ros_ign_bridge'>
      <command>roslaunch --wait marble_qav500_sensor_config_2 vehicle_topics.launch world_name:=#{_worldName} name:=#{_name}</command>
  </executable>
  HEREDOC
end
