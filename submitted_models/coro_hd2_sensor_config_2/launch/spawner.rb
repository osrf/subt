...
def rosExecutables(_name, _worldName)
  ...
  <executable name='robot_description'>
    <command>roslaunch --wait coro_hd2_sensor_config_1 description.launch world_name:=#{_worldName} name:=#{_name}</command>
  </executable>
  <executable name='topics'>
    <command>roslaunch --wait coro_hd2_sensor_config_1 vehicle_topics.launch world_name:=#{_worldName} name:=#{_name} breadcrumbs:=1 thermal:=1</command>
  </executable>
  HEREDOC
end
