def spawner(_name, _modelURI, _worldName, _x, _y, _z, _roll, _pitch, _yaw, _additionalSpawnPlugins="")
  joints = []
  for leg in ["front_left", "front_right", "rear_left", "rear_right"] do
...
def rosExecutables(_name, _worldName)
  ...
  <executable name='description'>
    <command>roslaunch --wait bosdyn_spot description.launch name:=#{_name}</command>
  </executable>
  <executable name='topics'>
    <command>roslaunch --wait bosdyn_spot vehicle_topics.launch world_name:=#{_worldName} name:=#{_name}</command>
  </executable>
  HEREDOC
end
