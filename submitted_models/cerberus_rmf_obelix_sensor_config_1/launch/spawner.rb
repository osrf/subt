...
def rosExecutables(_name, _worldName)
  <<-HEREDOC
    <executable name='rmf_obelix_description'>
      <command>roslaunch --wait cerberus_rmf_obelix_sensor_config_1 description.launch world_name:=#{_worldName} name:=#{_name}</command>
    </executable>
    <executable name='rmf_obelix_ros_ign_bridge'>
      <command>roslaunch --wait cerberus_rmf_obelix_sensor_config_1 vehicle_topics.launch world_name:=#{_worldName} name:=#{_name} uav:=1 lidar_3d:=1 laser_scan:=0 top_scan:=0 bottom_scan:=0 stereo_cam:=0 rgbd_cam:=0</command>
  </executable>
  HEREDOC
end
