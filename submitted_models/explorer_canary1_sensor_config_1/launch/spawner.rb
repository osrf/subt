def spawner(_name, _modelURI, _worldName, _x, _y, _z, _roll, _pitch, _yaw)
  ...
  <spawn name='#{_name}'>
      ...
      <pose>#{_x} #{_y} #{_z + 0.051} #{_roll} #{_pitch} #{_yaw}</pose>
      ...
      <plugin
        ...
        <maximumLinearAcceleration>2.1 2.1 4</maximumLinearAcceleration>
        <maximumLinearVelocity>5.5 5.5 1.6</maximumLinearVelocity>
        <maximumAngularVelocity>3 3 0.8</maximumAngularVelocity>
        ...
        <rotorConfiguration>
          <rotor>
            <jointName>rotor_0_joint</jointName>
            <forceConstant>5.121148e-05</forceConstant>
            <momentConstant>0.016</momentConstant>
...
def rosExecutables(_name, _worldName)
  <<-HEREDOC
    <executable name='robot_description'>
      <command>roslaunch --wait explorer_canary1_sensor_config_1 description.launch world_name:=#{_worldName} name:=#{_name}</command>
    </executable>
    <executable name='topics'>
      <command>roslaunch --wait explorer_canary1_sensor_config_1 vehicle_topics.launch world_name:=#{_worldName} name:=#{_name}</command>
  </executable>
  HEREDOC
end
