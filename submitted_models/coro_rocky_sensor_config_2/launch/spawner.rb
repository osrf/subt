  ...
  <spawn name='#{_name}'>
    ...
    <sdf version='1.6'>
      <include>
        ...
        <uri>#{_modelURI}</uri>
      <!-- Ackermann Steering -->
      <plugin filename="libackermannrocky.so"
        ...
        <wheel_separation>0.285</wheel_separation>
        <kingpin_width>0.22</kingpin_width>
        <steering_limit>0.266</steering_limit>
        ...
        <min_acceleration>-4</min_acceleration>
...
def rosExecutables(_name, _worldName)
  ...
  <executable name='robot_description'>
      <command>roslaunch --wait coro_rocky_sensor_config_1 description.launch world_name:=#{_worldName} name:=#{_name}</command>
    </executable>
    <executable name='topics'>
      <command>roslaunch --wait coro_rocky_sensor_config_1 vehicle_topics.launch world_name:=#{_worldName} name:=#{_name} breadcrumbs:=1</command>
    </executable>
  HEREDOC
end
