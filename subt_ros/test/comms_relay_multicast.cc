...
/////////////////////////////////////////////////
TEST(relay, multicast)
{
  // Test communication on multicast utilizing SubtRosRelay and a dummy broker.
  ...
  CommsClient sender("b", false, false, false, nh.get());
  advanceTime();
  ...
  // unicast
  ASSERT_FALSE(!c1.Bind(c1_cb, kMulticast, 123)); advanceTime();
  ASSERT_FALSE(!c1.Bind(c1_cb, "", 123)); advanceTime();
  ASSERT_FALSE(!c2.Bind(c2_cb, kMulticast, 123)); advanceTime();
  ASSERT_FALSE(!c3.Bind(c3_cb, "", 123)); advanceTime();
  ...
  for(unsigned int i=0; i < 15; ++i)
  {
    ...
    oss << "Hello clients, " << i;
    EXPECT_TRUE(sender.SendTo(oss.str(), kMulticast, 123));
  ...
  ASSERT_TRUE(c1.SendTo("Selftest", kMulticast, 123));

  sentData.emplace_back("Selftest");
  advanceTime();  // advance the test broker
  ros::spinOnce();

  EXPECT_EQ(1u, receivedData1.size());
