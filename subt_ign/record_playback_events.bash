...
if [ -z "$1" ]; then
  echo "Usage: bash ./record_playback_events.bash [path_to_log_dir] [path_to_ouput_directory]"
  exit 0
fi
...
export SUBT_EVENT_ID=
if [ $# -eq 3 ]; then
  export SUBT_EVENT_ID=$3
fi
...
echo "Starting $logDirPath" >> record_status.log

export IGN_GAZEBO_SYSTEM_PLUGIN_PATH=$LD_LIBRARY_PATH:$IGN_LAUNCH_PLUGIN_PATH
sdfName="playback_event_recorder"
IGN_TRANSPORT_TOPIC_STATISTICS=1 ign gazebo -s -v 4 $sdfName.sdf

echo "Video recording ended. Shutting down playback"

pgrep -f $sdfName | xargs kill -9 &> /dev/null

echo "Moving mp4 videos to dir: $videoDirPath"
mv *.mp4 $videoDirPath
