...
class subt::SensorsPrivate
{
  ...
  public: std::vector<ignition::sensors::Sensor *> cameras;
  ...
  /// \brief Connection to ignition::gazebo::events::Stop event, used to stop thread
  ...
  public: std::vector<ignition::sensors::RenderingSensor *> activeSensors;
  ...
  public: std::map<ignition::sensors::SensorId,
  ...
  public: ignition::math::Color color = ignition::math::Color::Black;
};
...
void SensorsPrivate::WaitForInit()
{
  while (!this->initialized && this->running)
  {
    ...
    if (this->doInit)
    {
      ...
      this->renderUtil.SetBackgroundColor(this->color);
      this->renderUtil.SetAmbientLight(ignition::math::Color::White);
...
void SensorsPrivate::RunOnce()
{
  ...
  if (!this->activeSensors.empty())
  {
...

    {
      ...
      // ignition::sensors::RenderingSensor::SetManualSceneUpdate and set it to true
...
//////////////////////////////////////////////////
PlaybackSensors::PlaybackSensors() : System(), dataPtr(std::make_unique<SensorsPrivate>())
{
}

//////////////////////////////////////////////////
PlaybackSensors::~PlaybackSensors()
{
  this->dataPtr->Stop();
}

//////////////////////////////////////////////////
void PlaybackSensors::Configure(const ignition::gazebo::Entity &/*_id*/,
    const std::shared_ptr<const sdf::Element> &_sdf,
    ignition::gazebo::EntityComponentManager &_ecm,
    ignition::gazebo::EventManager &_eventMgr)
{
  igndbg << "Configuring Sensors system" << std::endl;
  std::string type = _sdf->Get<std::string>("type","follow").first;

  if (type == "traj")
    this->dataPtr->color = ignition::math::Color::White;
  ...
  this->dataPtr->renderUtil.SetEnableSensors(true,
      std::bind(&PlaybackSensors::CreateSensor, this,
...
//////////////////////////////////////////////////
void PlaybackSensors::Update(const ignition::gazebo::UpdateInfo &_info,
                     ignition::gazebo::EntityComponentManager &_ecm)
{
...
//////////////////////////////////////////////////
void PlaybackSensors::PostUpdate(const ignition::gazebo::UpdateInfo &_info,
                         const ignition::gazebo::EntityComponentManager &_ecm)
{
  IGN_PROFILE("PlaybackSensors::PostUpdate");
  ...
  if (_info.dt < std::chrono::steady_clock::duration::zero())
  {
    ...
    for (auto cam : this->dataPtr->cameras)
    {
      cam->Init();
    }
  ...
  if (this->dataPtr->running && this->dataPtr->initialized)
  {
    ...
    auto t = ignition::math::secNsecToDuration(time.first, time.second);
    ...
    {
      ignition::sensors::Sensor *s = this->dataPtr->sensorManager.Sensor(id);
...
//////////////////////////////////////////////////
std::string PlaybackSensors::CreateSensor(
    ...
    const sdf::Sensor &_sdf, const std::string &_parentName)
{
  ...
  // Hack to only create a spawned_camera, so that other robot's don't
  // create new sensors.
  if (_sdf.Name().find("spawned_camera") == std::string::npos)
  {
    return std::string();
  }
  ...
  this->dataPtr->ecm->PinEntity(parentModel);
...
IGNITION_ADD_PLUGIN(subt::PlaybackSensors,
    ignition::gazebo::System,
  subt::PlaybackSensors::ISystemConfigure,
  subt::PlaybackSensors::ISystemUpdate,
  subt::PlaybackSensors::ISystemPostUpdate
)
